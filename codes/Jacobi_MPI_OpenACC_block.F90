    ...
    module commondata
        ...
        integer(kind=4), parameter :: itc_max=100000
        ...
        real(kind=8), allocatable :: send_pos(:), send_neg(:), recv_pos(:), recv_neg(:)
        !$acc declare create(A, Anew, AGlobal, send_pos, send_neg, recv_pos, recv_neg)
    ...
    integer(kind=4) :: num_gpus, gpu_id
    integer(kind=4) :: local_comm, local_rank
    integer(kind=4) :: name_len, tmp
...
!-----------------------------------------------------------------------------------------------------------------------
    local_rank = -1
    call MPI_Comm_split_type(MPI_COMM_WORLD, MPI_COMM_TYPE_SHARED, myID, MPI_INFO_NULL, local_comm, ierr)
    call MPI_Comm_rank(local_comm, local_rank, ierr)
    call MPI_Comm_free(local_comm, ierr)
    
    num_gpus = 0
    num_gpus = acc_get_num_devices(acc_device_nvidia)
    num_gpus = acc_get_num_devices(acc_device_nvidia)-1 !% Enforce use GPU0 (computing card) 
    if (local_rank.EQ.0) then
        write(*,*) "******We have", num_gpus, "GPU(s)******"
    endif
    if (num_gpus.LE.0) then
        if (myID_2D.EQ.0) then
            write(*,'(a)'), 'No NVIDIA GPUs available'
            call MPI_Abort(MPI_COMM_WORLD, 1, ierr)
        endif
    else
        gpu_id = mod(local_rank, num_gpus)
        write(*,*) "Local rank:", local_rank, "; myID_2D:", myID_2D, "; GPU_ID: ", gpu_id
...
!-----------------------------------------------------------------------------------------------------------------------
    ...
    ! allocate buffer array to store messages in x direction
    allocate(send_pos(1:nyLocal))
    allocate(send_neg(1:nyLocal))
    allocate(recv_pos(1:nyLocal))
    allocate(recv_neg(1:nyLocal))
    ...
    if(myID.EQ.0) then
        ...
        do iSrc = 1, nProc-1
            ...
            call MPI_RECV(tempA, (countX(iSrc)+2)*(countY(iSrc)+2), MPI_DOUBLE_PRECISION, iSrc, 110, MPI_COMM_2D, MPI_STATUS_IGNORE, ierr)
    ...
    !$acc parallel loop
    do j = 1, nyLocal
        send_pos(j) = A(nxLocal, j)
        send_neg(j) = A(1, j)
    enddo
    
    !$acc parallel loop collapse(2) async
    do j = iStartY+1, iEndY-1
        do i = iStartX+1, iEndX-1
    ...
    call MPI_SENDRECV(A(1, nyLocal), nxLocal, MPI_DOUBLE_PRECISION, nbr_top, 0, &
                                    ...
                                    MPI_COMM_2D, MPI_STATUS_IGNORE, ierr)
    call MPI_SENDRECV(A(1, 1), nxLocal, MPI_DOUBLE_PRECISION, nbr_bottom, 1, &
                                    A(1, nyLocal+1), nxLocal, MPI_DOUBLE_PRECISION, nbr_top, 1, &
                                    MPI_COMM_2D, MPI_STATUS_IGNORE, ierr)
    !$acc end host_data     
    
    !$acc host_data use_device(send_pos, send_neg, recv_pos, recv_neg)
    call MPI_SENDRECV(send_pos, nyLocal, MPI_DOUBLE_PRECISION, nbr_right, 0, &
                        recv_pos, nyLocal, MPI_DOUBLE_PRECISION, nbr_left, 0, &
                        MPI_COMM_2D, MPI_STATUS_IGNORE, ierr)
    call MPI_SENDRECV(send_neg, nyLocal, MPI_DOUBLE_PRECISION, nbr_left, 1, &
                        recv_neg, nyLocal, MPI_DOUBLE_PRECISION, nbr_right, 1, &
                                    MPI_COMM_2D, MPI_STATUS_IGNORE, ierr)    
    !$acc end host_data  
    ...
    !$acc parallel loop collapse(2) reduction(+:errorLocal)
    do j = iStartY, iEndY
        do i = iStartX, iEndX
